...
$proc.StartInfo.Arguments = "-jar D:\ServerBox\Servers\ColdFusion\cfusion\hf-updates\hotfix-012-315717.jar -i silent -f D:\ServerBox\Servers\ColdFusion\cfusion\hf-updates\hotfix-012-315717.properties"
...
#get isapi dirs
$dirs = Get-ChildItem -Path "D:\ServerBox\Servers\ColdFusion\config\wsconfig" -Directory
#path to isapi dll patch
$newIsapiPath = "\\isilon-hq-dfw\DevArchive\ServerBox\Servers\ColdFusion\config\wsconfig\isapi_redirect.dll"
foreach ($dir in $dirs) {
	$oldIsapiPath = $dir.FullName + "\isapi_redirect.dll"
	if (Test-Path $oldIsapiPath) {
		#backup current dll
		Copy-Item -Path $oldIsapiPath -Destination "$oldIsapiPath.bak" -Force
		#copy isapi patch into place
		Copy-Item -Path $newIsapiPath -Destination $oldIsapiPath -Force
	}
}
