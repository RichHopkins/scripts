[string]$currentVersinon = 'release/13.07'
[string]$newVersion = 'release/13.08'
...
$binaryDirs = @(
	...
	"\\isilon-hq-dfw\devintreo\inetpub\mks-binaries-devint\file_library\properties",
	"\\isilon-hq-dfw\devintreo\inetpub\mks-binaries-devint\report_temp",
	"\\isilon-hq-dfw\devintreo\inetpub\wwwroot\file_library\properties",
	"\\isilon-hq-dfw\devintreo\inetpub\mks-binaries-devint\file_library\properties",
...
$jenkinsDirs = @(Get-ChildItem -Path "\\txv12doeqnc01\d$\Jenkins\jobs" -Directory)
...
#Disable Alpha job
$config = [System.IO.File]::ReadAllText("\\txv12doeqnc01\d$\Jenkins\jobs\Alpha - Build\config.xml").Replace($enabledString, $disabledString)
[System.IO.File]::WriteAllText("\\txv12doeqnc01\d$\Jenkins\jobs\Alpha - Build\config.xml", $config)

#Update release.txt
$config = [System.IO.File]::ReadAllText("\\txv12doeqnc01\d$\workspace\config\Release.txt").Replace($currentVersinon.Replace('release/', ''), $newVersion.Replace('release/', ''))
[System.IO.File]::WriteAllText("\\txv12doeqnc01\d$\workspace\config\Release.txt", $config)
...
$config = [System.IO.File]::ReadAllText("\\cahwtvsel128\c$\acceptancesuite\Alpha\Resources\config.properties").Replace($currentVersinon.Replace('release/', ''), $newVersion.Replace('release/', ''))
[System.IO.File]::WriteAllText("\\cahwtvsel128\c$\acceptancesuite\Alpha\Resources\config.properties", $config)
...
"@
Invoke-Sqlcmd2 -ServerInstance "DEVINT_DSN_Reotrans" -Database "reotrans" -Query $query
Invoke-Sqlcmd2 -ServerInstance "ALPHA_DSN_Reotrans" -Database "reotrans" -Query $query
