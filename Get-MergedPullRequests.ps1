param (
	...
	[string]$outputFile = "C:\PullRequests.csv"
)
...
for ($i = 1; $i -le $projects.Count; $i++) {
	...
	for ($x = 1; $x -le $repos.Count; $x++) {
		$repo = $repos[$x - 1]
		(& $cli --server $server --token $cliToken --action getPullRequestList --project $project.Key --repository $repo.Name --file $outputFile --append --state MERGED) | Out-Null
	}
}
#Send-MailMessage -Attachments $outputFile -To "EQ_Dev.TeamLeads@equator.com; eq_atl.help@equator.com; _release.engineer@equator.com" -Subject "Daily Pull Request Report" -SmtpServer "smtp-dev" -From "eqDevOps@equator.com"