...
$eqServerRoot = 'https://eqalm.equator.com'
$hqServerRoot = 'https://eqalm.hq.reotrans.com'
$cliPath = 'D:\atlassian-cli'
$dbRoot = "D:\Database$environment"
...
function Read-GitChangeLog {
    ...
    for ($i = 0; $i -le ($log.Length - 1); $i++) {
        if ($log[$i] -match "commit .{40}") {
            ...
            $jiraKeys = (& $cliPath\bitbucket.bat --server $hqServerRoot/bitbucket --token $cliToken --action getCommit --project $project --repository $repository --id $commit | Select-String 'Jira keys . . . . . . . . . . : (. *)') -replace 'Jira keys . . . . . . . . . . : ', ''
        ...
        if (($log[$i] -match ":\d{6}") -and (($log[$i] -split "`t")[0].Substring(($log[$i] -split "`t")[0].get_Length()-1) -ne "D") -and (($log[$i] -split "`t")[0].Substring(($log[$i] -split "`t")[0].get_Length() - 4) -ne "R100")) {
            $file = ($log[$i] -split "`t")[1] -replace '/', '\'
            if (-not($changeLogEntriesTemp.scriptPath -match ("$projectRoot\$file").replace('\', '\\'))) {
...
function Send-DevErrorEmail {
    ...
    switch ($environment) {
        "DevInt|Alpha" {
            ...
            Send-MailMessage -SmtpServer "smtp-dev" -To $usermail -Cc "eqdevops@equator.com; $commitermail" -From "eqdevops@equator.com" -Subject $subject -Body $body -BodyAsHtml $true -Attachments $attach
        }
        "Beta|Stage|REM|Produection" {
            ...
            Send-MailMessage -SmtpServer "smtp-dev" -To $usermail -Cc "_release.engineer@equator.com; $commitermail" -From "_release.engineer@equator.com" -Subject $subject -Body $body -BodyAsHtml $true -Attachments $attach
        }
    }
}
...
function Invoke-SQLScript {
    ...
    } elseif ($scriptPath -match "sqljobs") {
        switch ($environment) {
            "DevInt" {$dbServer = "TXV12SQEQNC22"}
            "Alpha" {$dbServer = "TXV12SQEQNQ21"}
            "Beta"{}
            "REM"{}
            "Stage"{}
            "Production" {}
    ...
    $isError = Get-Content $sqlLogPath -WarningAction SilentlyContinue | Where-Object {
        $_ -match "Msg|HResult|Unexpected|Error"
    ...
    if ($isError) {
        ...
        #& $cliPath\jira.bat --server $eqServerRoot/jira --user $jiraUser --password $jiraPass --action createIssue --issueType "Bug" --project $jiraProject --summary $summary --description $description --assignee $author
        #ENTER CODE HERE THAT GETS THE BUG ID JUST CREATED AS $issueID
        #& $cliPath\jira.bat --server $eqServerRoot/jira --user $jiraUser --password $jiraPass --action addAttachment --issue $issueID --file $sqlLogPath
...
if ($changeLogEntries) {
    ...
    foreach ($key in ($changeLogEntries.jiraKeys | Select-Object -Unique)) {
        $attachCheck = (& $cliPath\jira.bat --server $eqServerRoot/jira --user $jiraUser --password $jiraPass --action getAttachmentList --issue $key) -replace "\d{1,2} attachments for issue: .*", '' | ConvertFrom-Csv
        if ($attachCheck.Name -eq "ReleaseInstructions.xml") {
            ...
            & $cliPath\jira.bat --server $eqServerRoot/jira --user $jiraUser --password $jiraPass --action getAttachment --issue $key --file "ReleaseInstructions.xml"
            Pop-Location
...
Write-Output $dtNextBuild | Out-File "D:\workspace\config\lastBuildTime.txt"
